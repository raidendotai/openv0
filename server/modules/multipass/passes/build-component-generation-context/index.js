...
async function run(req) {
  ...
  retrieved.components = retrieved.components.map( (library_component,idx) => {
    ...
    const _tokens_limit = parseInt(process.env.CONTEXT__COMPONENTS_LIBRARY_EXAMPLES__TOKEN_LIMIT)
    let _consumed_tokens = 0
    let _examples = []

    while ( (_consumed_tokens < _tokens_limit) && _library_component_examples.length ) {
      ...
      if ( _consumed_tokens < _tokens_limit ) _examples.push(random_component_example)
  ...
  const component_context = [
    ...retrieved.components.map( (e,idx)=>{
      const examples_block =  !e.docs.examples.length
                                ? ''
                                : '\n\n'
                                  + `# full code examples of ${_titleCase(req.query.framework)} components that use ${e.name} :\n`
                                  + e.docs.examples.map((example)=>{
                                    return '```' + example.source + '\n' + example.code.trim() + '\n```'
                                  }).join(`\n\n`)
      return {
        role: `user`,
        content: `Library components can be used while making the new ${_titleCase(req.query.framework)} component\n\n`
                + `Suggested library component (${idx+1}/${retrieved.components.length}) : ${e.name} - ${e.description}\n`
      ...
      retrieved.icons.icons.length
        ? {
          ...
          content: `Icon elements can optionally be used when making the React component.\n\n`
                    ...
                    + '```' + component_ext + '\n'
                    + `import { ${retrieved.icons.icons.map(e=> e.retrieved.join(' , ') ).join(' , ')} } from "${retrieved.icons.import}";`
                    + '\n```\n\n'
