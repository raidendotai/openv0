...
function error_badSyntax (query) {
  ...
  return  '## error type :\n'
        ...
        + query.error_data.code_with_line_numbers.split(`\n`).slice(
...
function error_missingImports (query) {
  ...
  return  '## error type :\n'
        ...
        + query.error_data.component_used_nodes.filter(
          _used_node => !query.error_data.component_imports.map(e=>e.imported).flat().includes(_used_node)
        ).map(e=>`* ${e}`).join(`\n`)
        + '\n---\n'
        + '## suggested fixes :\n'
...
function error_illegalImports (query) {
  ...
  const illegal_imports = query.error_data.component_imports.filter(
    c => Object.keys(
            query.error_data.component_imports_checks
          ).filter(
            k => !query.error_data.component_imports_checks[k]
          ).includes(c.from)
  )
          ...
          + illegal_imports.map(
            (e,idx) => {
              const suffix = e.imported.filter(e=> e!=`default`).length
                            ? `\n  * was used to import elements : ${e.imported.filter(e=>e!='default').join(' , ')}`
                            : ''
            return  `* non-allowed import (${idx+1}/${illegal_imports.length}) : ${e.from}` + suffix
            }
          ).join(`\n\n`)
        + '\n---\n'
        + '## suggested fixes :\n'
...
async function run(req) {
  ...
  if (req.pipeline.stages[`component-validation-check`].success) {
    ...
    const errors_context_entries = req.pipeline.stages[`component-validation-check`].data.validation_errors.map(
      (_validation_error , _error_idx) => {
      return {
        role: `user`,
        content: `# Component Error (${_error_idx+1}/${req.pipeline.stages[`component-validation-check`].data.validation_errors.length})\n` +
                  `---\n` +
    ...
    const context = [
      {
        ...
        content:
          `You are an expert at writing ${_titleCase(
            req.query.framework,
          )} components and fixing ${_titleCase(req.query.framework,)} code with errors.\n` +
      ...
      ...errors_context_entries,
      {
        ...
        content:
          ...
          `- Current ${_titleCase(req.query.framework)} component code which has errors :\n\n` +
          "```" + FRAMEWORKS_EXTENSION_MAP[req.query.framework] + "\n" +
          req.pipeline.stages[`component-validation-check`].data.code +
          "\n```\n\n" +
          `Rewrite the full code to fix and update the provided ${_titleCase(req.query.framework)} web component\n` +
