...
async function build(){
  ...
  const db = fs.readdirSync(`./build/gits/themesberg$flowbite-svelte/src/routes/docs/components`)
  ...
  .map( (doc_file) => {
    ...
    const codeBlocks = index.split(split_sub)[1].split(`##`).slice(1,)
    ...
    return {
      ...
      docs: {
        ...
        use : [ codeBlocks[0] ].filter(block=>!block.source.includes(`custom`)),
        examples: codeBlocks.slice(1,).filter(block=>!block.source.includes(`custom`)),
      }
    }
  }).filter(e=>e)


  fs.writeFileSync( `./library/components/svelte/flowbite/dump.json`, JSON.stringify(db) )
  console.dir({
