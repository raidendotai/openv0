...
async function build(){
  ...
  const examples_files = fs.readdirSync(
    `./build/gits/huntabyte$shadcn-svelte/apps/www/src/lib/registry/default/example`
  ).filter(e=>e.includes(`.svelte`))

  const db = fs.readdirSync(`./build/gits/huntabyte$shadcn-svelte/apps/www/src/content/components`)
  ...
  .map( (doc_file) => {
    try{
      ...
      let meta = {}
      index.split('---')[1].trim().split('\n').map((e)=>{
        const split = e.trim().split(':').map(_e=>_e.trim())
        if (split[0] === `title`) meta.title = split[1]
        else if (split[0] === `description`) meta.description = split[1].slice(0,-1)
      ...
      const examples = examples_files.filter( f => f.startsWith(slug) ).map( (f) => {
        //console.dir({slug,f})
        return {
          source: f,
      ...
      return {
        ...
        docs : {
          ...
          use : svelteBlocks.length > 1
                  ...
                  : [{source : `${doc_file.split(`.`)[0]}.svelte`, code : svelteBlocks[0]}],
          examples,
        }
      }
    }catch(e){
  ...
  fs.writeFileSync( `./library/components/svelte/shadcn/dump.json`, JSON.stringify(db) )
  console.dir({
