...
async function pre_build(){
  ...
  return docs_files/*.filter(e=>e.includes(`navbar`))*/.map(doc_file=>{
    ...
    const useHead = eval(
              ...
              .filter(l=> ( !( (l.includes(`import`)) && (l.includes(`./`))))) // ghost local imports
              .join(`\n`)
              .split(`export default function App() {`)[0]
    const use = [
      {
        source: `usage.svelte`,
        code : [useHead , useBody]
      }
...
async function build() {
  console.dir({skip : `svelte/nextui`})
  // const pre_db = await pre_build()
  // const db = pre_db
  // console.dir(db[0].docs , {depth:null})
  // process.exit(0)
  /*
