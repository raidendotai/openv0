...
const generatedComponents = (await import(
  "./generated_components_dump.json"
)) as unknown as GeneratedComponentsFile;
...
export default function GeneratedComponentsList() {
  ...
  useEffect(() => {
    ...
    const imports = generatedComponents.openv0_components?.map(async (item) => {
      ...
      const importPath = `./openv0_generated/${item.name}/${
        item.versions.slice(-1)[0]
      }.tsx`;
      ...
      try {
        ...
        let module;
        try {
          module = await import(importPath /* @vite-ignore */);
        } catch (e) {
          return false;
        }
        return {
    ...
    try {
      fetch(
        `http://localhost:3000/component/ping/?from=Home (Browse Components)`,
      );
    } catch (e) {
      console.log(e);
    }
