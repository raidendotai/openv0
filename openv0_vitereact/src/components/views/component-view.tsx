  ...
  useEffect(() => {
    const getComponents = async () => {
      try {
        ...
        setComponentIdOg(data.componentId);

        const imports = data.iterations.map(async (item) => {
          return {
            prompt: item.prompt,
          ...
          <div className="max-w-8xl mx-auto px-12 pt-6 grid xl:grid-cols-12 items-start">
            <div className="xl:col-span-10">
              <div
                className={
                  darkModeToggle
                    ? "dark bg-black text-white px-6 py-1 pb-4"
                    : "px-6 py-1 pb-4"
                }
              >
                <a
                      ...
                      <div className="overflow-auto">
                        {generatedComponentImports
                          ...
                          .map((item, index) => (
                            <p
                              key={index}
                              className="whitespace-pre-wrap font-mono p-4 text-xs border cursor-pointer hover:p-6 hover:text-pink-600 duration-200"
                              onClick={() => {
                  ...
                  <Card>
                    <CardHeader className="border-b border-opacity-50">
                      <CardTitle>
                        ...
                        <span className="underline">{componentIdOg}</span>
                      </CardTitle>
                      <CardDescription>
                        Make changes to your component. This will iterate on the
            ...
            <div className="xl:col-span-2 grid grid-cols-3 xl:grid-cols-1 max-h-screen items-start overflow-y-auto border-l border-b border-r pb-12 rounder rounded-xl shadow shadow-xl xl:max-w-[25vw] overflow-x-auto">
              ...
              {generatedComponentImports
                ...
                .map((item, index) => (
                  ...
                  >
                    <div
                      className="p-1 px-2 m-2 mb-0 break-words text-sm cursor-pointer duration-200 xl:max-w-[13vw] overflow-x-auto"
                      onClick={() => {
                        setSelectedComponentIndex(
                          generatedComponentImports.length - 1 - index
                        );
                    ...
                    >
                      <p
                        className={
                          darkModeToggle
                            ? "dark text-white text-xs whitespace-pre-wrap"
                            : "text-black text-xs whitespace-pre-wrap"
                        }
                      >
                        • {item.prompt}
                        <br />
