...
const rag_icons = require(`./rag_icons.js`)
...
async function run(query){
  ...
  retrieved.library_components = retrieved.library_components.map( (e,idx) => {
    ...
    const examples_total_tokens = tiktokenEncoder.encode(
      e.docs.examples.map(example=> example.source + '```\n' + example.code.trim() + '\n```' ).join('\n\n')
    ).length

    console.log(`tokens for context entry ${e.name} : ${examples_total_tokens} (limit : ${process.env.CONTEXT_TOKENS_PER_LIBRARY_COMPONENT_LIMIT})`)

    if (examples_total_tokens > parseInt(process.env.CONTEXT_TOKENS_PER_LIBRARY_COMPONENT_LIMIT) ) {
      let updated_library_component = {...e}
      updated_library_component.docs.examples = [ e.docs.examples[Math.floor(Math.random()*e.docs.examples.length)] ]
    ...
    ...retrieved.library_components.map( (e,idx)=>{
      return {
        ...
        content: `Library components can be used while making the new React component\n\n`
                + `Suggested library component (${idx+1}/${retrieved.library_components.length}) : ${e.name} - ${e.description}\n`
